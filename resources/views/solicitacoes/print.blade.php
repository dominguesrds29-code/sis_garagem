...
<body>
    ...
    <div style='text-align: justify'>
        <p style="margin-top: .5cm;"><strong>Número:</strong> {{ $solicitacao->id }} / SELM / {{ date('Y') }}</p>
        <p><strong>Autorizo o motorista:</strong>
            {{ $motorista ? "{$motorista->name} {$motorista->grade} {$motorista->specialty}" : "Desconhecido [ID: {$solicitacao->motorista_id}]" }}
        </p>
        <p><strong>Portador da carteira de habilitação Nr:</strong>
            {{ $motorista ? $solicitacao->motorista->cnh_number : '--' }} </p>
        <p><strong>A sair com a viatura:</strong>
            {{ $solicitacao->viatura ? $solicitacao->viatura->modelo : "Desconhecida [ID: {$solicitacao->viatura_id}]" }}
        </p>
        <p><strong>Para a cidade de:</strong> {{ $solicitacao->destino }} </p>
        <p><strong>Às</strong> {{ $solicitacao->hora_inicio }} hs <strong>do dia</strong>
            {{ $solicitacao->getFormatDate($solicitacao->dt_inicio) }}</p>
        ...
        <p style="text-align: right; margin-top: 1cm;">São José dos Campos,
            {{ $solicitacao->getFormatDate(date('Y-m-d', strtotime($solicitacao->created_at))) }}</p>
        @if (auth()->check())
            ...
            <table style="width: 100%;">
                ...
                <tr>
                    <td style="text-align: center;"> {{ auth()->user()->name }} {{ auth()->user()->pst_specialty }}
                        <br> Despachante da SELM
                    </td>
                    <td></td>
                    <td style="text-align: center;"> Jorge Henrique de Oliveira de Godoy Cap Esp CTA <br> Comandante do
                        DTCEA-SJ</td>
                </tr>
            </table>
            </p>
        @endif
        <p style="margin-top: 1cm; text-align: center;">CONTROLE DA SELM</p>
        ...
        <table style="margin-top: .8cm; width: 100%; line-height: 16pt;">
            <tr>
                <td style="border-bottom: 1px solid #000;">Ocorrências:</td>
            </tr>
            @for ($i = 0; $i < 8; $i++)
                <tr>
                    <td style="border-bottom: 1px solid #000; height: 13pt;"></td>
                </tr>
            @endfor
        </table>
    </div>
